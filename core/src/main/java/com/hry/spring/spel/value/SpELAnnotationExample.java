...
public class SpELAnnotationExample {
	 ...
	 *  T也可以用于调用静态方法
	 */
	// T指定java.lang.Class实例: 输出 -->
	@Value("#{ T(java.util.Date)}")
	private Class<?> tDate;
	
	// 对于java.lang下面的类， T()可以不指定全限定名称: 输出 -->
	@Value("#{ T(String)}")
	 ...
	 * 	访问对象的属性，调用对象的方法
	 */
	// 访问对象inventor的属性name: 输出 -->
	@Value("#{ inventor.name }")
	private String propertiesGetObjectMethod;
	
	// 调用对象inventor的成员变量birthdate的getYear()方法: 输出 -->
	 ...
	 * maps
	 */
	// 获取inventor对象的成员变量map的mapKey_2的值: 输出 -->
	@Value("#{ inventor.map[mapKey_2]}")
	private int mapKeyValue;
	
	// 注入初始化map对象. {}表示空map: 输出 -->
	 ...
	 * 	
	 */
	// 如果systemProperties['pop3.port']没有值，通过?指定默认值，避免抛出NullPointerException: 输出 -->
	...
	public boolean istStaticMethod() {
